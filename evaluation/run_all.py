...
**Deployed model: `{cls_data['deployed_model']['algorithm']}`** (PR-AUC {cls_data['deployed_model']['pr_auc']:.3f}) — `ml/training/train_classifier.py` benchmarks both candidates above and deploys whichever wins on PR-AUC rather than always deploying RandomForest.
...
|---|---|---|---|---|---|---|---|---|
| PR-AUC | {cls_data['deployed_model']['pr_auc']:.3f} | {cls_data['deployed_model']['algorithm']} (deployed) | Synthetic Sensors | Machine-wise Holdout | `eval_classification.py` | `classification.json` | YES | RandomForest scored {cls_data['random_forest']['pr_auc']:.3f} PR-AUC in the same benchmark; the pipeline deploys whichever wins, not RandomForest by default. |
