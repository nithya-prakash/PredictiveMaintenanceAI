...
def evaluate_shap():
    ...
    try:
        classifier_artifact = joblib.load("models/classifier.pkl")
        classifier = classifier_artifact["model"]
        algorithm = classifier_artifact["algorithm"]
        background = classifier_artifact.get("background_sample")
    ...
    # and we want the positive class [1]. LinearExplainer (LogisticRegression)
    # returns the positive-class contribution directly.
